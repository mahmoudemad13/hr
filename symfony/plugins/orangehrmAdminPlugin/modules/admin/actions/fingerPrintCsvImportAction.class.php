 ...
 */
class fingerPrintCsvImportAction extends baseCsvImportAction {
	...
	public function execute($request) {
		...
		$this->setForm(new FingerPrintCsvImportForm());

		if ($request->isMethod('post')) {

			$this->form->bind($request->getParameter($this->form->getName()), $request->getFiles($this->form->getName()));
			$file = $request->getFiles($this->form->getName());
			if ($_FILES['fingerPrintCsvImport']['size']['csvFile'] > 1024000 || $_FILES == null) {
                $this->getUser()->setFlash('warning', __('Failed to Import: File Size Exceeded'));
				$this->redirect('admin/fingerPrintCsvImport');
			}
